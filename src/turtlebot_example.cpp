...
void visualize_milestones() {
    cout << "milestones = [";
    for (int i = 0; i < num_milestones; i++){
        cout << Milestones(i, 0) << " " << Milestones(i, 1) << "; ";
    }
    cout << "];" << endl;
}
...
void generate_edges() {
    ...
    for (i = 0; i < num_milestones; i++) {
        for (j = 0; j < i; j++) {
            ...
            if ((Milestones(i, 0) == Milestones(j, 0)) && (Milestones(i, 1) == Milestones(j, 1))) {
...
// Function to perform initial probability road map operation
void perform_prm() {
	generate_milestones();
    if (DEBUG_MODE) {
